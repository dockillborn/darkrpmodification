...
local function AnimationMenu()
    ...
    for k,v in SortedPairs(Anims) do
        ...
        button:SetPos(10, (i - 1) * 45 + 30)
        button:SetSize(180, 35)
        button:SetText("")

        button.Paint = function(self)
            if self:IsHovered() then
                draw.RoundedBox( 0, 0, 0, self:GetWide(), self:GetTall(), glitchhhud_config.backgroundCol )
                draw.SimpleText( v, "Button_Font", self:GetWide()/3, self:GetTall()/5, Color(255,255,255,100), TEXT_ALIGN_CENTER )
            else
                draw.RoundedBox( 0, 0, 0, self:GetWide(), self:GetTall(), glitchhhud_config.backgroundCol )
                draw.SimpleText( v, "Button_Font", self:GetWide()/3, self:GetTall()/5, Color(255,255,255,255), TEXT_ALIGN_CENTER )
            end
        end
